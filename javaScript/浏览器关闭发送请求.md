...
## 使用什么方式进行请求
+ `img` 动态创建img 利用 `src`属性进行通知,同理script的src应该也可以
+ `XMLHttpRequest` 同步发送ajax请求（上述例子便是如此操作）
+ `navigator.sendBeacon()` [sendBeacon](https://developer.mozilla.org/en-US/docs/Web/API/Navigator/sendBeacon)  这个方法主要用于满足统计和诊断代码的需要，这些代码通常尝试在卸载（unload）文档之前向web服务器发送数据
